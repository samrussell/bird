...
of_dump_all(void)
{
  ...
  WALK_LIST(i, oface_list)
    of_dump(i);
  debug("Router ID: %08x\n", config->router_id);
}

static inline unsigned
of_what_changed(struct iface *i, struct iface *j)
{
  unsigned c;
...
static inline void
of_copy(struct iface *to, struct iface *from)
{
...
static inline void
ofa_send_notify(struct proto *p, unsigned c, struct ifa *a)
{
  if (p->ifa_notify)
    {
      if (p->debug & D_IFACES)
	log(L_TRACE "%s < %s address %I/%d on interface %s %s",
      ...
      log_msg(L_DEBUG "ofa_send_notify() called");
      log_msg(L_DEBUG "Added route to %u.%u.%u.%u/%d", ((unsigned char *)(&(a->prefix)))[3], ((unsigned char *)(&(a->prefix)))[2], ((unsigned char *)(&(a->prefix)))[1], ((unsigned char *)(&(a->prefix)))[0], a->pxlen);
      if(a->iface){
...
static void
ofa_notify_change_dep(unsigned c, struct ifa *a)
{
  struct proto *p;

  DBG("OFA change notification (%x) for %s:%I\n", c, a->iface->name, a->ip);

  WALK_LIST(p, active_proto_list)
    ofa_send_notify(p, c, a);
}

static inline void
ofa_notify_change(unsigned c, struct ifa *a)
{
  neigh_ifa_update(a);
  ofa_notify_change_dep(c, a);
}

static inline void
of_send_notify(struct proto *p, unsigned c, struct iface *i)
{
  if (p->if_notify)
    {
      if (p->debug & D_IFACES)
...
static void
of_notify_change(unsigned c, struct iface *i)
{
  struct proto *p;
  struct ifa *a;
  ...
  if (c & IF_CHANGE_DOWN)
	...
	ofa_notify_change_dep(IF_CHANGE_DOWN, a);
      }

  WALK_LIST(p, active_proto_list)
    of_send_notify(p, c, i);

  if (c & IF_CHANGE_UP)
    WALK_LIST(a, i->addrs)
      {
	a->flags = (i->flags & ~IA_FLAGS) | (a->flags & IA_FLAGS);
	ofa_notify_change_dep(IF_CHANGE_UP, a);
      }
...
static unsigned
of_recalc_flags(struct iface *i, unsigned flags)
{
...
static void
of_change_flags(struct iface *i, unsigned flags)
{
  unsigned of = i->flags;
...
of_update(struct iface *new)
{
  ...
  WALK_LIST(i, oface_list)
    if (!strcmp(new->name, i->name))
      {
	new->addr = i->addr;
	new->flags = of_recalc_flags(new, new->flags);
	c = of_what_changed(i, new);
	...
	of_copy(i, new);
	if (c)
	  of_notify_change(c, i);

	i->flags |= IF_UPDATED;
	return i;
      }
  i = mb_alloc(of_pool, sizeof(struct iface));
  memcpy(i, new, sizeof(*i));
  init_list(&i->addrs);
newif:
  init_list(&i->neighbors);
  i->flags |= IF_UPDATED | IF_TMP_DOWN;		/* Tmp down as we don't have addresses yet */
  add_tail(&oface_list, &i->n);
  return i;
}

void
of_start_update(void)
{
  struct iface *i;
  struct ifa *a;
...
void
of_end_partial_update(struct iface *i)
{
  if (i->flags & IF_TMP_DOWN)
    of_change_flags(i, i->flags & ~IF_TMP_DOWN);
}
...
of_end_update(void)
{
  ...
  WALK_LIST(i, oface_list)
	...
	of_change_flags(i, (i->flags & ~IF_ADMIN_UP) | IF_SHUTDOWN);
      else
	{
	  WALK_LIST_DELSAFE(a, b, i->addrs)
	    if (!(a->flags & IF_UPDATED))
	      ofa_delete(a);
	  of_end_partial_update(i);
	}
    }
}

void
of_flush_ifaces(struct proto *p)
{
  if (p->debug & D_EVENTS)
...
of_feed_baby(struct proto *p)
{
  ...
  WALK_LIST(i, oface_list)
    {
      of_send_notify(p, IF_CHANGE_CREATE | ((i->flags & IF_UP) ? IF_CHANGE_UP : 0), i);
      if (i->flags & IF_UP)
	WALK_LIST(a, i->addrs)
	  ofa_send_notify(p, IF_CHANGE_CREATE | IF_CHANGE_UP, a);
    }
}
...
struct iface *
of_get_by_name(char *name)
{
  struct iface *i;

  if (i = of_find_by_name(name))
    return i;

  /* No active iface, create a dummy */
  i = mb_allocz(of_pool, sizeof(struct iface));
...
static int
ofa_recalc_primary(struct iface *i)
{
...
void
ofa_recalc_all_primary_addresses(void)
{
  struct iface *i;
...
static inline int
ofa_same(struct ifa *a, struct ifa *b)
{
...
ofa_update(struct ifa *a)
{
  ...
  b = mb_alloc(of_pool, sizeof(struct ifa));
...
ofa_delete(struct ifa *a)
{
  ...
  WALK_LIST(b, i->addrs)
	...
	if (b->flags & IF_UP)
	  {
	    b->flags &= ~IF_UP;
	    ofa_notify_change(IF_CHANGE_DOWN, b);
	  }
	if (b->flags & IA_PRIMARY)
	  {
	    of_change_flags(i, i->flags | IF_TMP_DOWN);
	    ofa_recalc_primary(i);
	  }
	mb_free(b);
	return;
      }
...
u32
of_choose_router_id(struct iface_patt *mask, u32 old_id)
{
#ifndef IPV6
  struct iface *i;
  struct ifa *a, *b;
  ...
  WALK_LIST(i, oface_list)
      ...
      WALK_LIST(a, i->addrs)
	{
	  ...
	  if (mask && !oface_patt_match(mask, i, a))
	    continue;
...
of_init(void)
{
  of_pool = rp_new(&root_pool, "Interfaces");
  init_list(&oface_list);
  neigh_init(of_pool);
}
...
int
oface_patt_match(struct iface_patt *ifp, struct iface *i, struct ifa *a)
{
...
struct iface_patt *
oface_patt_find(list *l, struct iface *i, struct ifa *a)
{
  struct iface_patt *p;

  WALK_LIST(p, *l)
    if (oface_patt_match(p, i, a))
      return p;
...
static int
oface_plists_equal(struct iface_patt *pa, struct iface_patt *pb)
{
...
int
oface_patts_equal(list *a, list *b, int (*comp)(struct iface_patt *, struct iface_patt *))
{
